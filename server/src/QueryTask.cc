...
void QueryTask::process()
{
	...
	cout << "Task Processing Target:" << _queryWord 
		<< "size = " << _queryWord.size() 
		<< "sockfd=" << _conn->getSockfd() << endl;
...
void QueryTask::queryIndex(const string &sin)
{
	...
	cout << sin << ":" << _conn->getSockfd() << " "
		<< pthread_self() << " " 
		<< _resultque.size() << _queryWord.substr(0, _queryWord.size())
		<< "|||" << endl;
}
