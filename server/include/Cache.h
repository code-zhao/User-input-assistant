...
template <class K = string, class D = string> //有一个默认值
struct Node
{
	K _query;
	D _jsonfile;
	Node *prev, *next;
	Node(K k, D d):_query(k),_jsonfile(d),prev(NULL),next(NULL){}
};
...
void Cache<K, D>::set(K key, D data)
{
	...
	else
	{
		...
		if(_cacheMap.size() == _size)
		{
			...
			remove(_tail);  //remove只是移动没有delete
			delete todel;
